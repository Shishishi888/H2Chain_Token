...
{
    public class ManagerListContractContractPrivilegeTest : ManagerListContractContractPrivilegeTestBase
    {
        [Fact]
        public async Task CreateProposal_WithPrivileged_Test()
        {
            ...
            var result = await otherTester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.CreateProposal),
                createProposalInput);
            result.Status.ShouldBe(TransactionResultStatus.Failed);
            result.Error.Contains("Unauthorized to propose.").ShouldBeTrue();
            
            //verify with view method
            var byteString = await otherTester.CallContractMethodAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.ValidateProposerInWhiteList),
        ...
        public async Task CreateProposal_Creator_Test()
        {
            ...
            var result = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.CreateProposal),
                createProposalInput);
        ...
        {
            var result = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.ChangeOrganizationProposerWhiteList),
        ...
        public async Task Change_OrganizationProposalWhiteList_Test()
        {
            var organizationAddress = await GetDefaultOrganizationAddressAsync();
            var result = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.GetProposerWhiteList), new Empty());
            ...
            var proposalInput = CreateManagerListContractProposalInput(proposerWhiteList, organizationAddress);
            var createResult = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.CreateProposal),
                proposalInput);
            createResult.Status.ShouldBe(TransactionResultStatus.Mined);
            var proposalId = Hash.Parser.ParseFrom(createResult.ReturnValue);
            await ManagerListContractMemberApprove(proposalId);
            var releaseResult = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.Release), proposalId);
            releaseResult.Status.ShouldBe(TransactionResultStatus.Mined);

            result = await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.GetProposerWhiteList), new Empty());
        ...
        [Fact]
        public async Task ValidateAddressIsManagerListContractMember_Test()
        {
            //miner member
            var byteResult = await Tester.CallContractMethodAsync(
                ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.ValidateAddressIsManagerListContractMember),
        ...
        private async Task<Address> CreateOrganizationAsync(bool proposerAuthorityRequired = false)
        {
            ...
            var transactionResult =
                await Tester.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                    nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.CreateOrganization),
        ...
        private async Task<Address> GetDefaultOrganizationAddressAsync()
        {
            var result = (await Tester.ExecuteContractWithMiningAsync(
                ManagerListContractAddress,
                nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.GetDefaultOrganizationAddress),
                new Empty()));
            return Address.Parser.ParseFrom(result.ReturnValue);
        }

        private async Task ManagerListContractMemberApprove(Hash proposalId)
        {
            var miner = Tester.CreateNewContractTester(Tester.InitialMinerList[0]);
            (await miner.ExecuteContractWithMiningAsync(ManagerListContractAddress,
                    nameof(ManagerListContractContractImplContainer.ManagerListContractContractImplStub.Approve), proposalId)).Status
