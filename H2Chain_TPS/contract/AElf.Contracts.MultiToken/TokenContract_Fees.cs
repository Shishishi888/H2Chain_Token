...
namespace AElf.Contracts.MultiToken
{
    public partial class TokenContract
    {
        ...
        {
            AssertTransactionGeneratedByPlugin();  // 保证该交易由plugin生成，防止恶意调用
            ...
            var fromAddress = Context.Sender;  // 获取执行合约的sender地址
            var methodFees = Context.Call<MethodFees>(input.ContractAddress, nameof(GetMethodFee), // 获取目标合约的methodfees
                new StringValue {Value = input.MethodName});
            var successToChargeBaseFee = true;
            if (methodFees != null && methodFees.Fees.Any())  // 收取base费
            {
            ...
            if (methodFees != null && !methodFees.IsSizeFeeFree)  // 收取size费
            {
            ...
            // Update balances.
            foreach (var tokenToAmount in bill.FeesMap)  // bill.FeeMap 记录了method fee与size fee所有的费用信息，遍历该map，扣除从sender的地址上扣除相应symbol的费用
            {
                ModifyBalance(fromAddress, tokenToAmount.Key, -tokenToAmount.Value);  // 收取sender的每一项token
                Context.Fire(new TransactionFeeCharged  // 将把费用的信息记录到该交易所在的block的缓存中
                {
        ...
        /**
         * ClaimTransactionfees是干嘛的？？？
         */
        public override Empty ClaimTransactionFees(TotalTransactionFeesMap input)  
        {
            AssertSenderIsCurrentMiner();  // 保证扣取交易费的sender是当前的矿工
            ...
            Assert(claimTransactionExecuteHeight < Context.CurrentHeight,
                $"This method already executed in height {State.ClaimTransactionFeeExecuteHeight.Value}");  // 保证该交易在每个块中只执行一次，防止恶意调用
            ...
            foreach (var bill in input.Value)
            {
                ...
                ModifyBalance(Context.Self, symbol, amount);   // 遍历前一次收取的TotalTransactionFeesMap，将各个token转入Token Contract Address中
                TransferTransactionFeesToFeeReceiver(symbol, amount);  // burn其中10%的token，其余的token转到设置的receiver address下。
                                                                       // 在main chain的环境下，receiver为Treasury Contract Address，side chain的receiver如果没有定义的话则burn剩下的token。 
            }
        ...
        private void TransferTransactionFeesToFeeReceiver(string symbol, long totalAmount)
        {
            ...
            var burnAmount = totalAmount.Div(10);  // 设置燃烧交易费的比例
            if (burnAmount > 0)
            ...
            var transferAmount = totalAmount.Sub(burnAmount);  // 减掉燃烧掉的交易费
            if (transferAmount == 0)
                return;
            var treasuryContractAddress =
                Context.GetContractAddressByName(SmartContractConstants.TreasuryContractSystemName);
