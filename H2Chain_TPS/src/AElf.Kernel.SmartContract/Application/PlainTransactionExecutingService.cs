...
namespace AElf.Kernel.SmartContract.Application
{
    public class PlainTransactionExecutingService : IPlainTransactionExecutingService, ISingletonDependency
    {
         ...
         *      BlockHeader 交易所属区块的区块头
         *      Transaction 需要执行的交易列表
         *      PartialBlockStateSet 同一个块內先执行的交易产生的数据
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                var groupStateCache = transactionExecutingDto.PartialBlockStateSet.ToTieredStateCache(); // 根据PartialBlockStateSet生成缓存
                var groupChainContext = new ChainContextWithTieredStateCache(  // 根据交易dto的前一个区块的哈希值和块高查询executive
                ...
                foreach (var transaction in transactionExecutingDto.Transactions) // 循环执行每一笔交易
                {
                    TransactionTrace trace;
                    if (cancellationToken.IsCancellationRequested)  // 如果CancellationToken超时，停止执行交易
                        break;

                    var singleTxExecutingDto = new SingleTransactionExecutingDto // 生成单笔交易Dto
                    {
                        Depth = 0,
                    ...
                    try
                    {
                        var transactionExecutionTask = Task.Run(() => ExecuteOneAsync(singleTxExecutingDto,
                            cancellationToken), cancellationToken);  // 执行单笔交易
                    ...
                    if (!TryUpdateStateCache(trace, groupStateCache))  // 如果不能更新缓存，退出程序
                        break;
#if DEBUG
...
#endif
                    var result = GetTransactionResult(trace, transactionExecutingDto.BlockHeader.Height);  // 获取交易的执行结果

                    var returnSet = GetReturnSet(trace, result);  // 生成每个执行交易的returnSet：将交易哈希值、交易结果状态，bloom以及交易结果存入returnSet
                                                                    // 如果交易、pre交易、inline交易、post交易都成功，则将这些交易产生的数据存入returnSet；否则只将pre交易、post交易
                                                                        // 产生的数据存入returnSet
                    returnSets.Add(returnSet);  // 将returnSet放入returnSets列表
                }

                return returnSets;  // 返回returnSets
            }
            catch (Exception e)
            {
        ...
        protected virtual async Task<TransactionTrace> ExecuteOneAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (singleTxExecutingDto.IsCancellable)  // 如果交易是可以删除的
            ...
            try
            {
                executive = await _smartContractExecutiveService.GetExecutiveAsync(  // 执行交易
            ...
            try
            {
                ...
                if (singleTxExecutingDto.Depth == 0)  // 如果交易depth等于0，则有pre交易，执行pre交易，并且逐个执行。pre交易是啥？？？
                {
                ...
                if (singleTxExecutingDto.Depth == 0)  // 执行post交易。post交易是啥？？？
                {
