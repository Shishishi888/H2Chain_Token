...
namespace AElf.Kernel.SmartContract.Application
{
    public class SmartContractExecutiveService : ISmartContractExecutiveService, ISingletonDependency
    {
        ...
        public async Task<IExecutive> GetExecutiveAsync(IChainContext chainContext, Address address)
        {
            ...
            var pool = _smartContractExecutiveProvider.GetPool(address); // 合约执行器缓存池
            var smartContractRegistration = await GetSmartContractRegistrationAsync(chainContext, address); // smartContractRegistration包含合约的code

            if (!pool.TryTake(out var executive)) // 缓存池中没有执行器
            {
                executive = await GetExecutiveAsync(smartContractRegistration); // 生成目标合约的执行器
            }
            else if (smartContractRegistration.CodeHash != executive.ContractHash) // 缓存池中有执行器，但是不是目标合约的执行器
            {
                _smartContractExecutiveProvider.TryRemove(address, out _); // 这是在干嘛？？？
                executive = await GetExecutiveAsync(smartContractRegistration); // 生成目标合约的执行器
            }
