            ...
            IReadOnlyList<ServiceDescriptor> descriptors, ITransactionContext transactionContext)
        {
            try
            {
                ...
                var tokenContractAddress = await _smartContractAddressService.GetAddressByContractNameAsync(chainContext,
                    TokenSmartContractAddressNameProvider.StringName);  // 获取代币合约地址
                ...
                if (!IsTargetAcsSymbol(descriptors) && transactionContext.Transaction.To != tokenContractAddress)  // 这是在干嘛？？？
                {
                    return new List<Transaction>();
                }
                ...
                var txCost = await _txFeeService.CalculateFeeAsync(transactionContext, chainContext);  // 计算交易费
                ...
                var transactionSizeFeeSymbols =
                    await _transactionSizeFeeSymbolsProvider.GetTransactionSizeFeeSymbolsAsync(chainContext);  // 获取size费
